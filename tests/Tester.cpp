...
TEST_CASE("Runtime Errors") {
    ...
    for (uint i = 0; i < 100; i++) {
        ...
        for (uint j = 0; j < stageCount; j++) {
            ...
            handler.createEngine(name, valRange(gen), valRange(gen));
        }
        ...
        for (uint j = 0; j < stageCount; j++) {
            ...
            if (random > 23) {
                ship->addStage(valRange(gen), valRange(gen), handler.getEngine(name));
            } else if (random > 16) {
                ship->addStage(valRange(gen), valRange(gen), handler.getEngine("S" + std::to_string(i) + "." + std::to_string(j)));
            } else {
                ...
                ship->addStage(valRange(gen), valRange(gen),
        ...
        for (uint j = 0; j < stageCount; j++) {
            if (0 < modifyValChance(gen)) {
                ship->setStageDryMass(j, valRange(gen));
            }
            if (0 < modifyValChance(gen)) {
                ship->setStageFuelMass(j, valRange(gen));
            }
            if (0 < modifyValChance(gen)) {
